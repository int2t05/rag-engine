...
| 文档详情 | `…/[id]/documents/[docId]` | 单文档信息 | `GET …/documents/{docId}` |
...
- **清理临时文件**：`POST /api/knowledge-base/cleanup`（全局清理，不仅限当前 KB）。
...
4. **发送消息**：`POST /api/chat/{chatId}/messages`，body：`{ messages: [{role, content}, …] }`（**完整历史**，最后一条须为用户）；响应为 **SSE**，前端按行解析 `data:` JSON，累积 `text` 与引用。
5. **删除**：`DELETE /api/chat/{id}`；批量 `POST /api/chat/batch-delete`，body：`{ chat_ids: number[] }`。
